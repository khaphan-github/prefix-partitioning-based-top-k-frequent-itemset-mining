"""
Multiprocessing Runner Module for PTF Algorithm

Provides simplified interface for running the multiprocessing PTF algorithm variant.
All functions here use PrefixPartitioningbasedTopKAlgorithmMultiprocessing internally.
"""
...
from ptf.algorithm_multiprocessing import PrefixPartitioningbasedTopKAlgorithmMultiprocessing
from ptf.utils import write_output, track_execution, MetricsReporter


def run_ptf_algorithm_multiprocessing(
    file_path: str,
    top_k: int = 8,
    output_file=None,
    ...
    """
    Run PTF algorithm with partition-level parallelization using multiprocessing.

    This version uses multiple processes to process prefix-based partitions
    in parallel, providing true parallel execution on multi-core systems by
    bypassing Python's GIL.

    Algorithm:
    1. SEQUENTIAL: Initialize MH, rmsup, and promising item arrays
    2. PARALLEL: Process partitions using process pool (true parallelism)
    ...
    Args:
        ...
        num_workers: Number of processes. Default: os.cpu_count() (auto-detect)
    
    Example:
        >>> run_ptf_algorithm_multiprocessing("data/transactions.txt", top_k=8)
        >>> run_ptf_algorithm_multiprocessing("data/transactions.txt", top_k=8, num_workers=4)
    """
    with track_execution() as metrics:
        ...
        # Main algorithm - use MULTIPROCESSING variant
        ptf = PrefixPartitioningbasedTopKAlgorithmMultiprocessing(
            k=top_k,
...
def run_ptf_algorithm_multiprocessing_with_timing(
    file_path: str,
    top_k: int = 8,
    output_file=None,
    num_workers: int = None,
    ...
    """
    Run multiprocessing PTF algorithm and measure execution time.

    Args:
        file_path: Path to transaction database file
        top_k: Number of top-k itemsets to find
        output_file: Optional file object to write results to
        num_workers: Number of processes. Default: os.cpu_count()
    ...
    Example:
        >>> exec_time = run_ptf_algorithm_multiprocessing_with_timing("data.txt", top_k=8, num_workers=4)
        >>> print(f"Execution time: {exec_time:.4f}s")
    """
    with track_execution() as metrics:
        run_ptf_algorithm_multiprocessing(file_path, top_k, output_file, num_workers)
